...
class ROLLOUT(object):
    """
    generatorが途中まで生成したものを最後まで出力させる
    """
    ...
    def get_reward(self, sess, input_x, rollout_num, discriminator):
        ...
        外部から呼ばれるメソッド
        報酬を計算する
        @param rollout_num サンプリング回数
        """
        rewards = []
        for i in range(rollout_num):
            # given_num between 1 to sequence_length(20) - 1 for a part completed sentence
            # given_num:何単語目か
            for given_num in range(1, self.sequence_length):
                # given_numまで与えられたときのrollout
                feed = {self.x: input_x, self.given_num: given_num}
                samples = sess.run(self.gen_x, feed)
                # given_numまで与えられたときのrolloutから報酬計算
